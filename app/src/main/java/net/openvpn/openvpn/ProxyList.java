...
public class ProxyList {
    ...
    public ProxyList(String none_name) {
        if (none_name == null) {
    ...
    public void load() {
        try {
            if (this.backing_file != null) {
                ProxyList pl = unpersist((JSONObject) new JSONTokener(FileUtils.readFileAppPrivate(this.context, this.backing_file)).nextValue(), this.none_name);
                this.list = pl.list;
    ...
    public void save() {
        try {
            if (this.dirty && this.backing_file != null) {
                FileUtils.writeFileAppPrivate(this.context, this.backing_file, persist().toString(4));
                this.dirty = false;
            }
        } catch (Exception e) {
